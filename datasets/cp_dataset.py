...
class CPDataset(data.Dataset):
    ...
    def get_input_cloth(self, index):
        ...
        tensor
        """
        cloth_path = self._get_input_cloth_path(index)
    ...
    def _get_input_cloth_path(self, index):
        ...
        """
        c_name = self._get_input_cloth_name(index)
    ...
    def get_input_cloth_mask(self, index):
        """from cp-vton"""
        mask_path = self._get_input_cloth_mask_path(index)
    ...
    def _get_input_cloth_mask_path(self, index):
        c_name = self._get_input_cloth_name(index)
    ...
    def _get_person_image_path(self, index):
        im_name = self._get_person_image_name(index)
        image_path = osp.join(self.data_path, "image", im_name)
        return image_path

    def get_person_parsed(self, index):
        """ loads parsing image """
        parsed_path = self._get_person_parsed_path(index)
    ...
    def __getitem__(self, index):
        ...
        result = {
            "c_name": self._get_input_cloth_name(index),  # for visualization
            "im_name": self._get_person_image_name(index),  # for visualization or ground truth
