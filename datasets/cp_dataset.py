...
class CPDataset(data.Dataset):
    ...
    def get_target_worn_cloth(self, im, _parse_array):
        ...
        im_c = im * _parse_cloth_mask + (
                    1 - _parse_cloth_mask)  # [-1,1], fill 1 for other parts
        return im_c
    ...
    def get_input_person_pose(self, index):
        ...
        pose_map = torch.zeros(point_num, self.fine_height,
                               self.fine_width)  # constructs an N-channel map
        ...
        for i in range(point_num):
            ...
            if pointx > 1 and pointy > 1:
                draw.rectangle((pointx - r, pointy - r, pointx + r, pointy + r),
                               'white', 'white')
                pose_draw.rectangle((pointx - r, pointy - r, pointx + r, pointy + r),
                                    'white', 'white')
    ...
    def get_input_person_body_shape(self, _parse_array):
        ...
        _parse_shape = _parse_shape.resize((self.fine_width, self.fine_height),
                                           Image.BILINEAR)
